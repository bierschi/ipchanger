...
class Logger:
    ...
    USAGE:
            Logger(name='pyhidentity')

    """
    __instance = None

    def __init__(self, name='pyhidentity', level='info', log_folder='/var/log', log_file_size=10000000, debug=False):
    ...
    def __create_log_folder(self, log_folder):
        """creates log folder in '/var/log/pyhidentity'

        """
        try:

            if log_folder.endswith('/'):
                if not os.path.exists(log_folder + 'pyhidentity'):
                    os.mkdir(log_folder + 'pyhidentity')
            else:
                if not os.path.exists(log_folder + '/pyhidentity'):
                    os.mkdir(log_folder + '/pyhidentity')
